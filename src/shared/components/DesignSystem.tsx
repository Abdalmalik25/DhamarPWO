...
export const Typography = {
  ...
  WhiteSectionTitle: ({ children, subtitle }: { children: ReactNode; subtitle?: string }) => (
    <div className="text-center mb-12">
      <h2 className="text-3xl md:text-4xl font-black text-white mb-4">{children}</h2>
      {subtitle && (
        <p className="text-white/60 text-sm md:text-base max-w-2xl mx-auto">{subtitle}</p>
      )}
    </div>
  ),
  ...
  GraySectionTitle: ({ children, subtitle }: { children: ReactNode; subtitle?: string }) => (
    <div className="text-center mb-12">
      <h2 className="text-3xl md:text-4xl font-black text-gray-800 mb-4">{children}</h2>
      {subtitle && (
        <p className="text-gray-500 text-sm md:text-base max-w-2xl mx-auto">{subtitle}</p>
      )}
    </div>
  ),
...
export const Cards = {
  ...
  StatsCard: ({
    ...
    <div className="group bg-white/10 backdrop-blur-md rounded-2xl p-5 text-center hover:bg-white/20 transition-all duration-300 border border-white/10 hover:border-white/30">
      <div
        className={`w-10 h-10 bg-gradient-to-br ${colorClass} rounded-xl flex items-center justify-center mx-auto mb-3 shadow-lg group-hover:scale-110 transition-transform`}
      >
        <Icon
          size={18}
  ...
  ServiceCard: ({
    ...
    >
      <div
        className={`w-14 h-14 bg-gradient-to-br ${colorClass} rounded-2xl flex items-center justify-center mb-4 shadow-lg group-hover:scale-110 transition-transform`}
      >
        <Icon
          size={24}
          className="text-white"
...
export const Buttons = {
  ...
  SecondaryGlass: ({ children, onClick }: { children: ReactNode; onClick?: () => void }) => (
    <button
      onClick={onClick}
  ...
  PrimaryGov: ({ children, onClick }: { children: ReactNode; onClick?: () => void }) => (
    <button
      onClick={onClick}
...
export const Sections = {
  ...
  Content: ({ children, className = '' }: { children: ReactNode; className?: string }) => (
    <div className={`max-w-7xl mx-auto px-4 py-12 ${className}`}>{children}</div>
  ),

  // قسم التذييل الداكن (Dark Footer Section)
  DarkFooter: ({ children, className = '' }: { children: ReactNode; className?: string }) => (
    <div
      className={`bg-gradient-to-br from-gov-700 via-gov-800 to-gov-900 rounded-3xl p-8 md:p-12 text-white relative overflow-hidden shadow-xl ${className}`}
    >
      {/* خلفية زخرفية */}
...
export const Containers = {
  ...
  // حاوية البطاقات
  CardsGrid: ({ children, cols = 'md:grid-cols-3' }: { children: ReactNode; cols?: string }) => (
